...
class TransparencyController extends BaseApiController
{
    public function index(): ResponseInterface
    {
        ...
        $one = fn (string $s, array $binds = []) => (float) ($db->query($s, $binds)->getFirstRow('array')['v'] ?? 0);
        ...
        $awardsByDistrict = $db->query("SELECT d.name AS district, COUNT(a.id) AS awards, COALESCE(SUM(a.amount),0) AS value
            ...
            WHERE a.standstill_until < ? GROUP BY d.id, d.name ORDER BY value DESC", [$now])->getResultArray();

        return $this->ok([
            'published_notices' => $one("SELECT COUNT(*) v FROM notices WHERE status='published'"),
            'total_awarded_value' => $one("SELECT COALESCE(SUM(amount),0) v FROM awards WHERE standstill_until < ?", [$now]),
            ...
            'open_notices' => $one("SELECT COUNT(*) v FROM notices WHERE status='published' AND closing_at > ?", [$now]),
            'closed_notices' => $one("SELECT COUNT(*) v FROM notices WHERE status='published' AND closing_at <= ?", [$now]),
